...
const debug = require('debug')('loopback:connector:mongodbsib');
const MongoPool = require('nodejs-libraries/mongoutils');
...
MongoDB.prototype.connect = function(callback) {
  const self = this;
  if (self.db) {
    debug("URL %s from self.db", self.settings.url);
    process.nextTick(function() {
    ...
    self.dataSource.once('connected', function() {
      process.nextTick(function() {
        debug("URL %s after connected self.db", self.settings.url);
        if (callback) callback(null, self.db);
      });
    });
  } else {
    ...
    const mongoPoolOptions = {
      service: 'loopback-sib',
      keepAlive: true,
      mongoOptions: validOptions,
    };
    // new mongodb.MongoClient(self.settings.url, validOptions).connect(function(
    (new MongoPool(self.settings.url, mongoPoolOptions)).get(function(
      err,
      client,
    ) {
      if (err) {
      ...
      return urlParser(self.settings.url, self.settings, function(err, url) {
        ...
        debug('Dbname: %s', url.dbName);
        self.db = client.db(
          url.dbName || self.settings.database,
          url.db_options || self.settings,
        );
        debug("URL %s DB initialized from client", self.settings.url);
